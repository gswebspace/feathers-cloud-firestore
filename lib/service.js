  ...
  async _find(params = {}) {
    ...
    for (const key in query) {
      if (query.hasOwnProperty(key)) {
        ...
        if (typeof queryValue !== "undefined" && queryValue !== null && typeof queryValue !== "object") {
          //Equality
          ...
          for (const operatorkey in queryValue) {
            if (queryValue.hasOwnProperty(operatorkey)) {
              const operatorval = queryValue[operatorkey];

              //Check supported $ operators
              if (operatorkey === "$in") {
                if (Array.isArray(operatorval) && operatorval.length > 1) {

                  if (operatorval.length > 10) {
                    throw new BadRequest("More than 10 values for $in are not supported")
                  }

                  if (!finalQuery) {
                    finalQuery = ref.where(key, 'in', operatorval);
                  } else {
                    finalQuery = finalQuery.where(key, 'in', operatorval);
                  }

                } else {
                  throw new BadRequest("$in requires multiple entries")
                }
              } else if (operatorkey === "$nin") {
                throw new BadRequest("$nin is not supported")
              } else if (operatorkey === "$ne") {
                throw new BadRequest("$ne is not supported")
              } else if (operatorkey === "$lt") {
                if (!finalQuery) {
                  finalQuery = ref.where(key, '<', operatorval);
                } else {
                  finalQuery = finalQuery.where(key, '<', operatorval);
                }
                ...
                throw new NotImplemented("$or is not yet supported")
              }
            }
          }

        } else {
          throw new BadRequest("Unknown query value type")
        }
      }
    }
    ...
    //$skip aka offset
    let offset = filters.$skip;
    if (offset) {
      if (!finalQuery) {
        finalQuery = ref.offset(offset);
      } else {
